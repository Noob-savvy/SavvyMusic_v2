...
from SavvyMusic.utils.inline.play import panel_markup_1, stream_markup, telegram_markup
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Savvy.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        if "live_" in queued:
            ...
            try:
                await Savvy.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_9"])
            if videoid == "telegram":
                button = telegram_markup(_, chat_id)
